...
Config ProgramOptions::parseCommandLine(int argc, char* argv[])
{
    try
    {
        ...
        options.add_options()
                ...
                ("texture-width", "texture width, default value is 256", cxxopts::value<std::uint32_t>(config.textureSize.w)->default_value("256"))
                ("texture-height", "texture height, default value is 256", cxxopts::value<std::uint32_t>(config.textureSize.h)->default_value("256"))
